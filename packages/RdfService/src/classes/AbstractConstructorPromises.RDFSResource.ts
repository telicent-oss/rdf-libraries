/*
 * @module RDFSResource @remarks
 * A wrapper class for an RDFS Resource - i.e. typed node in the graph   @author Ian Bailey
 */
import { AbstractConstructorPromises } from './AbstractConstructorPromises';
export { createQueryResponseSchema } from './../utils/createQueryResponseSchema';
export { SparQLResultBinding } from './../schema';
import { isEmptyString } from './../utils/isEmptyString';
import { CountQuerySolution, LiteralPropertyQuerySolution, LongURI, RelatedLiterals, RelatedNodeQuerySolution, ResourceDescription, SPOOSQuerySolution, StringsDict, TypedNodeQuerySolution, XsdDataType } from '../types';
...
export class RDFSResource extends AbstractConstructorPromises {
    ...
    */ 
    async getLiteralsList(options: { key:string; validate?: (value:unknown) => void}):Promise<string[]> {
      if (!options.key) throw new Error(`Expected key to be set, instead got ${options.key}`);
