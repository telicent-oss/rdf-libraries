...
function normalizeQueryEndpoint(endpoint: string): string {
  ...
  if (base.endsWith("/update")) {
    throw new Error(
      `Looks like an UPDATE endpoint, not a query endpoint: ${endpoint}`
    );
  }
  return `${base}/query`;
}
...
export async function triplesForSelect(
  ...
  if (typeof input !== "string") {
    ...
    if (!selResp.ok) {
      throw new Error(
        `SELECT failed ${selResp.status}: ${await selResp.text()}`
      );
    }
  ...
  for (let i = 0; i < iris.length; i += chunkSize) {
    ...
    if (!conResp.ok) {
      throw new Error(
        `CONSTRUCT failed ${conResp.status}: ${await conResp.text()}`
      );
    }
