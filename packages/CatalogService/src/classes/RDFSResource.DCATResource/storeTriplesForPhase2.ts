...
export const storeTriplesForPhase2: StoreTripleForOntology = async ({
  ...
  const asErrorValueObject = (error: unknown, meta: unknown) => ({
    error: `[${property}] ${error} ${JSON.stringify(meta)}`,
  });
  ...
  for (const operation of operations) {
    ...
    try {
      ...
      } else if (operation.type === "update" && "updateByPredicateFns" in api) {
      ...
      } else if (operation.type === "create" && "createByPredicateFns" in api) {
        // create
