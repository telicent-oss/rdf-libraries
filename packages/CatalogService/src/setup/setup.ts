...
export const setup = async (options: {
  ...
  let rdfWriteApiClient = options.rdfWriteApiClient;
  if (options.rdfWriteApiUri && rdfWriteApiClient === undefined) {
    rdfWriteApiClient = rdfWriteApiClientFactory({
      baseUrl: options.rdfWriteApiUri,
    });
  }
  if (rdfWriteApiClient === undefined) {
    throw new Error(
      `Invalid params - must set rdfWriteApiUri (${options.rdfWriteApiUri}) or rdfWriteApiClient (${options.rdfWriteApiClient})`
    );
  }
  ...
  return apiFactory(catalogService, rdfWriteApiClient, MOCK);
};
