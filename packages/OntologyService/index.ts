...
export class OntologyService extends RdfService {
  ...
  */
  async getAppliedStyles(classes: LongURI[] = []): Promise<AppliedStyle[]> {
    ...
    const styles: AppliedStyle[] = statements.map((statement: StylesQuerySolution) => {
      return new AppliedStyle(statement.cls.value, JSON.parse(decodeURIComponent(statement.style.value)))
    })
