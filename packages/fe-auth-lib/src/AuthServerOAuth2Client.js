...
class AuthServerOAuth2Client {
    ...
    constructor(config) {
        ...
        console.log(`🔧 Initialized ${this.isCrossDomain ? 'cross-domain' : 'same-domain'} OAuth2 client`);
    ...
    detectCrossDomain() {
        try {
            ...
            console.log(`🌐 Domain detection: current=${currentHost}, auth=${authServerHost}, sameDomain=${isSameDomain}`);
            return !isSameDomain;
        } catch (error) {
    ...
    async handleCallback(callbackParams = null) {
        ...
        console.log('🎉 Session created securely via domain cookies (access token never exposed)');
        ...
        if (this.isCrossDomain && sessionData.isCrossDomain && sessionData.sessionToken) {
            ...
            console.log('🔑 Cross-domain client: Session ID stored for Authorization header use');
        } else {
            console.log('❌ Cross-domain session storage skipped - conditions not met');
        }
    ...
    async isAuthenticated() {
        try {
            ...
            if (this.isCrossDomain && sessionId) {
                ...
                console.log('🔍 Using session ID in Authorization header for authentication check');
            }
            ...
            if (response.ok) {
                try {
                    ...
                    if (logoutData.external_logout && logoutData.logout_url) {
                        console.log('🔗 External IDP logout required, redirecting to:', logoutData.logout_url);
