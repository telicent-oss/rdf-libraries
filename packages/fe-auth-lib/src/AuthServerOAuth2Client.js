...
class AuthServerOAuth2Client {
  ...
  constructor(config) {
    ...
    this.config = {
      clientId: "change-me", // Default - should be overridden
      authServerUrl: "http://auth.telicent.localhost",
      redirectUri: "change-me", // Default - should be overridden
    ...
    try {
      ...
      const response = await fetch(
        `${this.config.authServerUrl}/session/check`,
        {
          ...
          signal, // abort fetch if component is unmounted
        }
      );
  ...
  afterRequest(response, url, options = {}) {
    if (response.status === 401) {
      ...
      if (
        ...
        this.login();
        throw new Error("Session expired");
      } else if (wwwHeader.includes("Malformed token")) {
